...
namespace WebView2_UWP.Pages
{
    public sealed partial class MainPage : Page
    {
        ...
        private void Page_GotFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (e.OriginalSource is MUXC.NavigationViewItem)
            {
                MUXC.NavigationViewItem item = (MUXC.NavigationViewItem)e.OriginalSource;
                if (!string.IsNullOrEmpty(item.Content.ToString()))
                    ElementName = item.Content.ToString();
                else
                {
                    if (!string.IsNullOrEmpty(item.Tag.ToString()))
                        ElementName = item.Tag.ToString();
                }
            }
            else
            {
                ...
                {
                    AppBarButton appBarButton = (AppBarButton)e.OriginalSource;
                    if (!string.IsNullOrEmpty (appBarButton.Label.ToString()))
                        ElementName = appBarButton.Label.ToString();
                }
                else
                    ElementName = (e.OriginalSource as FrameworkElement).Name;
            }

            string myFocus = "Focused on: " + ObjectType + "-->" + ElementName;
            Debug.WriteLine(myFocus);
            // Browser.StatusUpdate(myFocus);
        }
    }
}
